...
import vert from "../shadowsource.vert";
import frag from "../shadowsource.frag";

import shadowvert from "../shadowplane.vert";
import shadowfrag from "../shadowplane.frag";
...
class ShadowView implements View {
    ...
    private readonly planeCount = 6;
    private shadowPlanes: RenderTarget[];
    ...
    public constructor(gl: WebGL2RenderingContext) {
        ...
        for(let target = 0; target < this.planeCount; ++target) {
            this.shadowPlanes.push(new RenderTarget(gl, this.maxResolutionWidth/4.0, this.maxResolutionHeight/4.0))
        }
    ...
    render(aspect: number, volumeData: VolumeData, camera: Camera, settings: Settings): void {
        ...
        if(this.updateFps(camera, settings)) {
            ...
            for(let i = 0; i < this.shadowPlanes.length; ++i) {
                const target = this.shadowPlanes[i];
                ...
                if(i > 0) {
                    ...
                    gl.bindTexture(gl.TEXTURE_2D, this.shadowPlanes[i-1].getTexture());
                } 
                
                gl.activeTexture(gl.TEXTURE1);
                gl.bindTexture(gl.TEXTURE_3D, volumeData.texture);
                gl.uniform1i(this.shadowProgramInfo.uniformLocations.textureData, 1);
                
                gl.uniform3fv(this.shadowProgramInfo.uniformLocations.eyePos, eye);

                gl.uniform2f(this.shadowProgramInfo.uniformLocations.textureSize, target.getWidth(), target.getHeight());

                gl.uniform1f(this.shadowProgramInfo.uniformLocations.offset, i/(this.shadowPlanes.length-1));
                gl.uniform1i(this.shadowProgramInfo.uniformLocations.index, i);

                this.renderMesh.bindShader(gl, this.shadowProgramInfo.program);
            ...
            const i = Math.round((this.shadowPlanes.length-1) * depth);
            //for(let i = this.shadowPlanes.length-1; i >= 0; --i) 
            {
                const target = this.shadowPlanes[i];
                
                gl.activeTexture(gl.TEXTURE0);
                gl.bindTexture(gl.TEXTURE_2D, this.shadowPlanes[i].getTexture());
                ...
                //mat4.translate(transformed, transformed, vec3.fromValues(0.5, 0.5, -i/(this.shadowPlanes.length-1)));
